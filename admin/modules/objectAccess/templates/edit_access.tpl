        ...
        {capture assign=temp}
            {defun name=accessRuleRow template=false targetType="" targetId=0 value=0}
                <tr class="rule{if $template} template{elseif $targetType} rule{$targetType}{/if}">
                    ...
                    {capture assign=targetName}
                        {if $targetType=='General'}
                            {alias code=targetType`$targetType`}
                        {else}
                            {$subjects.$targetType.$targetId|escape}
                        {/if}
                    {/capture}
                    ...
                    <th class="name">
                        <label for="{$inputId}">{$targetName}</label>
                    </th>

                    <td class="value">
                        <label>
                            <input type="checkbox" name="{$inputId}" value="1" id="{$inputId}" {if !empty($value)} checked="checked"{/if} />
                        </label>
                    </td>

                    <td class="remove">
                        <button type="button" class="noStyling">
                            <img src="images/icons/delete.png" title="{alias code=removeDefinition}" alt="" />
                        </button>
                    </td>
