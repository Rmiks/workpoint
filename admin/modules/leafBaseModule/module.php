...
class leafBaseModule extends leaf_module
{
	...
	protected $mainObjectField = 'name';
	...
	public function getSubmenuGroupName()
	{
		if( is_null( $this->submenuGroupName ) )
		{
			...
			if( is_null( $this->submenuGroupName ) )
			{
				// disabled due to incorrect results
	...
	public function isAnyMenuItemActive( $items )
	{
	...
	protected function loadResourcePack( $name )
	{
		switch( $name )
		{
			...
			break;
			case 'richtext':
			    $this->loadResourcePack('dialog');
			    $this->loadResourcePack('objectlink');
			    $this->loadResourcePack('select');
				// richtext
			...
			break;
		    case 'googleMapPoint':
				$googleMapsKey = defined('googleMapsKey') ? googleMapsKey : '';
				if ( $googleMapsKey )
				{
					// enable APIs like Maps JavaScript API, Maps Distance Matrix API, Maps Embed API, ... if key used
					_core_add_js('//maps.googleapis.com/maps/api/js?key=' . $googleMapsKey);
				}
				else
				{
					_core_add_js('//maps.googleapis.com/maps/api/js?sensor=false');
				}
				break;
		   }
	}
    ...
    // 	if (!empty($_GET['do']) && $_GET['do'] == 'edit' || $_GET['do'] == 'view') {
    // 		$return[] = $this->mainObjectField;
    // 	}
