...
jQuery( document ).ready(function()
{
	...
	jQuery( '#moduleTree .secondaryPanel__title ' ).on('click', function()
	{
		var $nav = $('.secondaryPanel');
		var $item = $(this);
		var $parent = $item.parent();

		if (!$nav.hasClass('secondaryPanel--collapsed')) {			
			var cookieName = 'submenu:' + $parent.attr('data-title');
			
			if( $parent.hasClass( 'secondaryPanel__menuItem--collapsed' ) )
			{
				jQuery.cookie( cookieName, null );
			}
			else
			{
				jQuery.cookie( cookieName, 1, { expires: 30 } ); // expires in 30 days
			}

			$parent.toggleClass( 'secondaryPanel__menuItem--collapsed' );
		} else {
			var $firstItem = $parent.find('.secondaryPanel__submenuItem:first-child a');
			window.location.href = $firstItem.attr('href');
		}
	});
...
var leafBaseModule =
{
	...
	{
		// richtext config
		var plugins = [ 'paste', 'inlinepopups', 'iespell', 'insertdatetime', 'preview', 'searchreplace',
			'contextmenu', 'safari', 'leafImage', 'leafEmbed', 'leaflink', 'fullscreen' ];
		...
		var tinymceConfig = 
		{
			...
			theme_advanced_buttons1 : 'pastetext,pasteword,|,bold,italic,formatselect,justifyleft,justifycenter,justifyright,justifyfull,|,sub,sup,|,bullist,numlist,|,link,unlink,image,embed,|,code,cleanup,removeformat,|,fullscreen',
