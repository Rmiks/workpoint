...
function start_panel( target, object_id ){
	...
	if( active_object !== object_id )
	{
        ...
        panel.style.left = x_pos + "px";

		loading.style.top = y_pos + "px";
        loading.style.left = x_pos + "px";
...
{
    var ids = [];
    jQuery('#objectForm .secondaryPanel__checkbox input[type=checkbox]').each(function(){
       if (this.checked)
       {
...
jQuery(function()
{
	...
	jQuery( '.secondaryPanel__objectsForm' ).on( 'click', '.toggleChildren', function (event)
	{
		...
		$(target).parent().parent().toggleClass('secondaryPanel__menuItem--collapsed');
