...
class aliases extends leaf_module
{
    ...
    public $actions = [ 'save', 'delete', 'install', 'import' ];

    public $output_actions = [ 'edit', 'export', 'updateAliases', 'searchAliases' ];
    ...
    function __construct()
    {
        ...
        _core_add_js( SHARED_WWW . 'js/tableHeadFixer.js' );
  ...
  */
    ...
    function save()
    {
        ...
        if( $values[ 'id' ] )
        {
            $p->db_update_entry( 'translations_groups', $values, [ 'id' => $values[ 'id' ] ] );
        }
        else
        {
            unset( $values[ 'id' ] );
    ...
    function getTranslations( $groupId )
    {
        $result = [
            ...
            'translations'                     => [],
            'context'                          => null,
            'languagesWithMachineTranslations' => []
        ];
    ...
    /**
     * old txt export for reference
     *
     * @param array $translations
     */
    function exportText( $translations )
    {
        $context  = stringtolatin( $translations[ 'context' ], true, true );
    ...
    public static function mergeTranslationArrays( $groupId, $importTranslations, $existingTranslations = [] )
    {
        if( !isPositiveInt( $groupId ) )
        {
            return;
        ...
        foreach( $importTranslations as $importData )
        {
            ...
            {
                $machineArray = get( $importData, 'machine', [] );
    ...
    public function getContextCategory( $context )
    {
        if(
            ...
            ||
            ( in_array( $context, [ 'admin', 'pages_admin' ] ) )
        )
        {
            return 'admin';
        }
