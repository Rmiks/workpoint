...
class leafUser extends leafBaseObject implements leafUserInterface
{
    ...
    protected $objectRelations = [
        'group' => [ 'key' => 'group_id', 'object' => 'leafUserGroup' ],
    ];
        ...
        else
        {
            ...
            if( !$userIsLoggedIn )
            {
                if( $isLoginPost )
                {
                    ...
                    $userRow = dbGetRow( $userDataQp );
                    $loginOk = ( !empty( $userRow ) && password_verify( $_POST[ 'leafAuthPassword' ], $userRow[ 'password' ] ) );
    ...
    public function variablesSave( $variables, $fieldsDefinition = NULL, $mode = false )
    {
        ...
        {
            $fieldsDefinition[ 'oldpassword' ] = [];
            $fieldsDefinition[ 'password1' ]   = [];
            $fieldsDefinition[ 'password2' ]   = [];
            $p->addPostCheck( [ $this, 'checkNewPassword' ] );
        }
        elseif( $this->currentMode == 'admin' )
        {
            if( $this->id == 0 || isset( $variables[ 'change_password' ] ) )
            {
                $fieldsDefinition[ 'password1' ] = [];
                $fieldsDefinition[ 'password2' ] = [];
                $p->addPostCheck( [ $this, 'checkNewPasswordAdmin' ] );
            }
            $p->addPostCheck( [ $this, 'checkLogin' ] );
        }
    ...
    */
    public function checkNewPassword( $values )
    {
        ...
        //check old password
        if( !password_verify( $values[ 'oldpassword' ], $this->password ) )
        {
            $error[ 'field' ]     = [
                'name' => 'oldpassword'
            ];
    ...
    */
    ...
    {
        $p = new processing;
        $p->db_delete_entry( 'users_properties', [ 'user_id' => $this->id ] );
        $values = [];
