...
class leafFile extends leafBaseObject
{
    ...
    protected static function getRoot( $fileDataOrLeafFile )
    {
        ...
        {
            $root = call_user_func( [ $ownerClass, 'getFileRoot' ] );
        }
    ...
    public static function getCollection( $params = [], $itemsPerPage = null, $page = null )
    {
        $queryParts                = [];
        $queryParts[ 'select' ][]  = 'c.*';
        $queryParts[ 'from' ][]    = '`' . leafBaseObject::getClassTable( __CLASS__ ) . '` `c`';
    ...
    public function getCachedImages()
    {
        ...
        $ownerId    = $this->ownerObjectId;

        /** @var bundleBaseObject $parentObj */
        $parentObj = getObject( $ownerClass, $ownerId );
        if( $parentObj )
        {
            $parentRelations = $parentObj->getObjectRelations();

            // get name from parents object for this file
            $parentsGivenName = false;
            foreach( $parentRelations as $relationName => $relationDescription )
            {
                if( isset( $relationDescription[ 'object' ] ) && $relationDescription[ 'object' ] === 'leafFile' )
                {
                    // check if this file belongs to this relation
                    $key = $relationDescription[ "key" ];
                    if( $parentObj->$key == $this->id )
                    {
                        // relation matches
                        $parentsGivenName = $relationName;
                    }
                }
            }
            ...
            if( $parentsGivenName )
            {
                $imageSizes = $parentObj->getImageSizes();
                foreach( $imageSizes as $sizeName => $sizeProperties )
                {
                    $images[ $sizeName ] = [ 'path' => $parentObj->getImageUrl( $parentsGivenName, $sizeName, true ) ];
                }
            }
        }
