...
function dbQuery( $q, $dbLinkOrName = null, $returnId = null, $returnAffectedRows = null, $strictlyError = false, $ignoreSqlErrors = false )
{
    ...
    $trackQueries = leaf_get('properties', 'dbconfig', 'trackQueries');
    if( $trackQueries === true)
    {
    	$start = microtime();
	}	
    ...
    {
	    $end = microtime();
	    $length = stopWatch::calculate( $start, $end );
	    $GLOBALS['QUERY_LOG'][] = $length . ' : ' . trim(preg_replace('/(\n|\r|\s)+/', ' ', $q));
	}
    if( $returnId )
    {
...
class leafPDO extends PDO
{
    public static function connect( $dbConfig )
    {
      ...
      $connectString .= get( $dbConfig, 'port' ) ? ';port=' . get( $dbConfig, 'port' ) : '';
