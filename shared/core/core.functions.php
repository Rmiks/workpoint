...
function _core_init(){
	...
	if( DEV_MODE )
     {
         $errorPageHander = new Whoops\Handler\PrettyPageHandler();
     }
     else
     {
         $errorPageHander = new ErrorPageHandler();
     }
     
     $whoops = new Whoops\Run();
     $whoops->pushHandler( $errorPageHander );
     $whoops->pushHandler( new leafErrorHandler() );
     $whoops->register();
}
...
function _core_output(){
	...
	foreach($properties as $module_name => $module_config)
	{
		if($module_config['load'])
		{
			...
			{
				$load_function = $module_config[ 'load_function' ];
	...
	if ( $main_module )
	{
		...
		$load_function = $main_module[ 'load_function' ];
		return $module->$load_function( $modules_output );
	}
}
?>
