...
function removeFileObject () {
    jQuery('.fileobjectField__remove').on('click', function () {
        var text = jQuery(this).find('.fileobjectField__confirmMessage').val();
        if (confirm(text)) {
            var $parent = jQuery(this).closest('.fileobjectField') || jQuery(this).closest('.templateField');
            $parent.find('input.fileobject').val(0);
            $parent.find('.fileobjectField__content').remove();

            var $label = $parent.find('label.fileobjectField__input-label');
            if ($label.length) {
                if ($parent.find('input[type=file]').prop('multiple')) {
    ...
    Array.prototype.forEach.call( inputs, function( input )
    {
    	...
    	input.addEventListener( 'change', function( e )
    	{
            ...
            if (this.files && this.files.length > 1) {
                ...
                fileName = ( this.getAttribute('data-single-caption') || '' ).replace('[filename]',
                    '<b style="color: #37b1a7">' + inputFilename + '</b>');
            }
            else {
            ...
            $(this).closest('.fileobjectField').find('.fileobjectField__content').remove();
