{if $editor == "tinymce"}
	<textarea onchange="alert('changed')" rows="10" cols="50" class="leafRichtext" style="height:{if $field.properties.height}{$field.properties.height|escape}{else}500px{/if}; width: 98%;" id="{$field.input_id|escape}" name="{$field.input_name|escape}">{$field.value|trim|escape}</textarea>
{else}
