<select id="{$field.input_id|escape}"
    name="{$field.input_name|escape}{if $field.properties.multiple}[]{/if}"
    {if $field.properties.multiple} multiple="multiple"{/if}
    {if $field.properties.size>1} size="{$field.properties.size|escape}"{/if}
    class="{$className}"
>
