...
class xml_template extends leaf_object_module
{
	...
	function _assignObjectData(){
        ...
        foreach ($this->_properties['fields'] as $fieldName => $fieldDef) {
            $this->_loadFieldMapData($fieldDef);
        }

        if(isset($this->_properties['groups'])){
            foreach ($this->_properties['groups'] as $group){
    ...
    /**
     * Reads properties field and parses Google map positions into objects data arrays values
     * @param $field
     */
    function _loadFieldMapData($field){
        ...
        if ($field['type'] == 'google_map_point')
        {
        ...
        elseif ($field['type'] == 'array')
        {
            ...
            foreach ($field['fields'] as $subFieldName => $subFieldDef)
            {
                if ($subFieldDef['type'] != 'google_map_point' || empty($this->object_data['data'][$fieldName]))
                {
                    continue;
                }
	...
	function _typeSave($params, $save_values = null){
        ...
        $this->processLinkedFields($params);
	...
	function getFieldTypes(){
		$types = array();
        $fields = $this->_getMergedPropertyFields();
	...
	function _typeEdit($params){
		...
		if ( isset( $this->_properties[ 'groups' ] ) ) {
			foreach ( $this->_properties[ 'groups' ] as $key => $group )
			{
				$assign[ 'groups' ][ $key ] = $group;

				foreach ( $group[ 'fields' ] as $field ) {
					if ( $this->check_field_rules( $field ) )
					{
						$this->_loadFieldType( $field, $this->object_data[ 'template' ] );
						$assign[ 'groups' ][ $key ][ 'fields' ][ $field[ 'name' ] ] = $field;
					}
				}
			}
            foreach ( $assign[ 'groups' ] as &$group ) {
                $this->loadObjectFieldPreviews( $group['fields'] );
            }
		}
		...
		if (isset($fieldTypes['richtext']))
		{
			...
			_core_add_js(SHARED_WWW . 'classes/input/js/leafDialogStyling.js');
	...
	function _getArrayNode($params){
        ...
        $fieldArr = $this->_getMergedPropertyFields();

		$fields = $fieldArr[$_GET['item_name']]['fields'];
		foreach($fields as &$field)
		{
			if($this->check_field_rules($field))
			{
...
//type methods
	...
	function processLinkedFields(& $params, $arrayName = null, $arrayKey = null)
	{
        $fields = $this->_getMergedPropertyFields();
	...
	function _typeView($params = null, & $module = null){
		...
		if(file_exists($path_parts['dirname'] . '/' . $templateFile))
		{
            ...
            $template->register_outputfilter( [ 'alias_cache', 'fillInAliases' ] );
...
*/
	function saveTemplateDataToDb($data){
		...
		{
			//skip common fields
			if(empty($fields[$key]['common']) && !empty($fields[$key]))
			{
				// array have table
				if($fields[$key]['type'] == 'array' && !empty($this->arrayTables[$key]))
				{
                    if(!empty($val))
                    {
    ...
    /** Merges group and field properties in single array
     * @return array
     */
    function _getMergedPropertyFields(){
        $mergedFields = $this->_properties['fields'];
        if(isset($this->_properties['groups']) && is_array($this->_properties['groups'])){
            foreach( $this->_properties['groups'] as $group){
                $mergedFields = array_merge($mergedFields, $group['fields']);
            }
        }
        return $mergedFields;
    }
	function loadTemplateDataFromDb(){
		...
		if(!empty($this->table))
		{
			...
			if($entry = dbGetRow($q))
			{
				...
				{
					if($f_key != 'object_id' && isset($fields[$f_key]) && $fields[$f_key]['type'] == 'array' && $f_val)
					{
