...
function setAllAttribs(elm) {
	...
	if (tinyMCE.isMSIE5)
        ...
        // this allows to edit an existing link without deleting it first
        if (elm.hasAttribute('data-mce-href')) {
            elm.removeAttribute('data-mce-href');
        }
}
...
// setup leaf environment

var baseUrl;
var contentBaseUrl;
function initLeaf()
{
	baseUrl = tinyMCE.activeEditor.documentBaseURI.toAbsolute('?module=content&do=object_manager&type=newLinks');
	loadXmlHttp(baseUrl + '&setContentBaseUrl=1', setContentBaseUrl, null, 'get', null, 1);
	loadXmlHttp(baseUrl, loadObjectTree, null, 'get', null, 1);
}

function setContentBaseUrl(xml, params)
{
	contentBaseUrl = xml.responseText;
}

function loadObjectTree(xml, params)
{
	var objectTree = document.getElementById('objectTree');
	if(objectTree)
	{
		objectTree.innerHTML = xml.responseText;
	}
}
...
function pickObject(obj)
{
	...
	if(input)
	{
		input.value = contentBaseUrl + '?object_id=' + objId;
	}
}