...
// if true, then dbSE for all processed variables
    ...
    */
    ...
    public function reportError( $field, $errorCode, $errorResult = null )
    {
        ...
        if( !empty( $this->errorCallback ) )
        {
            ...
            )
            {
                ...
                $reflectionMethod->invokeArgs( $this->errorCallback[ 0 ], [ & $field, & $errorCode, & $errorResult, & $this ] );
            }
        }
    ...
    public function validateAndOutput( $definitions, $aliasContext = false, $variables = false, $format = null )
    {
        ...
        $result = $this->outputResult( $aliasContext, $format );
        return $result;
    }
    ...
    protected function outputVariables()
    {
        require_once( SHARED_PATH . 'classes/smarty_plugins/alias_cache.class.php' );
            ...
            if( $this->processing_fields[ $key ][ 'validateFunction' ] )
            {
                ...
                else
                {
                    ...
                    $params   = [
                        ...
                        $this->processing_fields[ $key ][ 'name' ]
                    ];
                }
    ...
    protected function _process_values()
    {
        foreach( $this->processing_fields as $key => $field )
        {
            ...
            {
                $this->processing_fields[ $key ][ 'value' ] = call_user_func( $this->processing_fields[ $key ][ 'post_func' ], $this->processing_fields[ $key ][ 'value' ], $this->processing_fields[ $key ][ 'name' ] );
            }
        }
    }
    ...
    protected function _prepare_values( $fields_definition, $variables = false, $return = false )
    {
        ...
        */
        ...
        foreach( $this->processing_fields as $key => $field )
        {
            ...
            if( is_string( $field ) )
            {
                $this->processing_fields[ $key ] = [ 'name' => $field ];
            }
    ...
    public function db_create_entry( $table, $fieldsOrRows, $triger_sql = false, $triger_ok = false, $escape = false, $add_quotes = false, $quote_fields = [] )
    {
    ...
    public function db_replace_entry( $table, $fieldsOrRows, $triger_sql = false, $triger_ok = false, $escape = false, $add_quotes = false, $quote_fields = [] )
    {
        $fieldsOrRowsErrorMessage = 'Processing error: invalid or empty $fieldsOrRows argument. ';
        $fieldsOrRowsErrorLevel   = E_USER_ERROR;
        if( $triger_sql )
    ...
    public function db_update_entry( $table, $fields, $where_q, $escape = true, $add_quotes = true, $quote_fields = [] )
    {
        //old way!
        if( is_array( $table ) )
        {
