...
class xmlize{
    ...
    /**
     * Create tables property fields from $properties array
     * @param $template_data
     * @param $properties
     * @param $table_keys
	...
	function checkTemplateTable($template_data, $properties, $table_keys, $arrayName = null){
		...
		else
		{
            ...
            // Add group fields for table column creation
            if( isset( $properties[ 'groups' ] ) && is_array( $properties[ 'groups' ] ) )
            {
	...
	function parseGroup( $group_data ) {
		foreach( $group_data[ '@' ] as $name => $value)
		{
			$field[ $name ] = $value;
		}
	...
	function recompileTemplate($template){
        ...
        //get all groups
		if ( isset( $template_data['*']['group'] ) )
		{
			foreach ( $template_data['*']['group'] as $group_key => $group_id )
			{
				$properties['groups'][$group_key] = $this->parseGroup( $template_data['#'][$group_id] );

				if ( isset( $template_data['#'][$group_id]['#'] ) )
				{
					$properties['groups'][$group_key]['fields'] = [];

					foreach ( $template_data['#'][$group_id]['#'] as $field_key => $field )
					{
						$properties['groups'][$group_key]['fields'][$field['@']['name']] = $this->parseField( $field );
		                ...
		                {
		                    $array_keys = $this->getTemplateKeys( $field );
		                    $properties['groups'][$group_key]['fields'][$field['@']['name']]['keys'] = $array_keys;
		                }
...
//            foreach($properties['groups'] as $group){
//                $properties['fields'] = array_merge($properties['fields'], $group['fields']);
//            }
//        }
