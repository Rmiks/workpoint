...
richtextImageDialog.prototype.createDialog = function()
{
	...
	self.widthInput 		= dialog.find( 'input[name=imageWidth]' );
	
	self.attributeElements = jQuery( [ self.altInput[0], self.titleInput[0], self.classSelect[0], self.widthInput[0] ] );
...
richtextImageDialog.prototype.attachEvents = function()
{
    ...
    self.widthInput.change( function()
    {
        self.dialog.find('img.preview').attr('width',  self.widthInput.val());
    });
	...
	self.dialog.find( '.insertButton' ).click(function()
	{
		if( self.previewImage.attr( 'src' ) )
		{
			...
			img.attr( 'width', 	self.widthInput.val() 			);
...
richtextImageDialog.prototype.updatePreview = function( settings )
{
    ...
    {
        self.widthInput.val(settings.imageWidth);
        self.previewImage.attr( 'width', settings.imageWidth );
    }
    else
    {
        self.widthInput.val( '' );
